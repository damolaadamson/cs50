//C program that runs a runoff election
...
int find_min(void)
{
    ...
    {
        if (candidates[l].votes < min_votes && candidates[l].eliminated == false)
        {
            min_votes = candidates[l].votes;
        }
    }
    return min_votes;
}
...
bool is_tie(int min)
{
    ...
    for (m = 0; m < candidate_count; m++)
    {
        ...
        else if (candidates[m].eliminated == false)
        {
            most_votes += 1;
        }
    }
